...
// 1/2021: Chris Rorden added (optional) Laplacian smooth
...
struct vec3f
{
    ...
    inline vec3f normalize( double desired_length = 1 )
    {
		...
		square = fmax(square, DBL_EPSILON);
...
namespace Simplify
{
	...
	void laplacian_smooth_iter(std::vector<vec3f> &vtx)
	//n.b. this function reads and writes array `vtx`, and does not modify `vertices`
	{
		//sum of neighboring vertices
		std::vector<vec3f> vSum(vertices.size());
		memset(vSum.data(), 0, sizeof(vec3f)*vertices.size());
		//number of neighboring vertices
		std::vector<int> vNum(vertices.size());
		memset(vNum.data(), 0, sizeof(int)*vertices.size());
		loopi(0,triangles.size())
		{
			//each point of a triangle has two neighbors:
			int p0 = triangles[i].v[0];
			int p1 = triangles[i].v[1];
			int p2 = triangles[i].v[2];
			vNum[p0] += 2;
			vSum[p0] = vSum[p0] + vtx[p1] + vtx[p2];
			vNum[p1] += 2;
			vSum[p1] = vSum[p1] + vtx[p0] + vtx[p2];
			vNum[p2] += 2;
			vSum[p2] = vSum[p2] + vtx[p0] + vtx[p1];
		}
		loopi(0,vertices.size()) //mean location of neighbors
		{
	...
	void laplacianHC_smooth(double alpha = 0.1, double beta = 0.5, int iter =10, bool lockBorderVertices = true)
	...
	//  trimesh.smoothing.filter_humphrey(mesh, alpha=0.1, beta=0.5, iterations=10) https://trimsh.org/trimesh.smoothing.html 
	{
		...
		update_mesh(0); //initialize
		loopi(0,triangles.size())
			triangles[i].deleted=0;
		std::vector<vec3f> o(vertices.size());
		std::vector<vec3f> p(vertices.size());
		std::vector<vec3f> q(vertices.size());
		std::vector<vec3f> b(vertices.size());
		loopi(0,vertices.size())
			o[i] = vertices[i].p;
		memcpy(p.data(), o.data(), sizeof(vec3f)*vertices.size());
		loopj(0,iter) {
			memcpy(q.data(), p.data(), sizeof(vec3f)*vertices.size());
			laplacian_smooth_iter(p);
			loopi(0,vertices.size())
				b[i] = p[i] - ((o[i] * alpha) + (q[i] * alpha1));
			memcpy(q.data(), b.data(), sizeof(vec3f)*vertices.size());
			laplacian_smooth_iter(q);
			loopi(0,vertices.size())
				p[i] = p[i] - ((b[i] * beta) + (q[i] * beta1));
		}
...
#ifdef use_meshify
	void get_raw(vec3d** pts, vec3i** tris, int *npt, int *ntri){
		int nt = triangles.size();
		vec3i *ts = (vec3i *) malloc(nt * sizeof(vec3i));
		int np = vertices.size();
		vec3d *ps = (vec3d *) malloc(np * sizeof(vec3d));
		loopi(0,np)
		{
			ps[i].x = vertices[i].p.x;
			ps[i].y = vertices[i].p.y;
			ps[i].z = vertices[i].p.z;
		}
		loopi(0,nt)
		{
			ts[i].x = triangles[i].v[0];
			ts[i].y = triangles[i].v[1];
			ts[i].z = triangles[i].v[2];
		}
		*pts = ps;
		*tris = ts;
		*npt = vertices.size();
	...
	void load_raw(vec3d* pts, vec3i* tris, int npt, int ntri){
		vertices.clear();
		triangles.clear();
		int material = -1;
		Triangle t;
		...
		{
			Vertex v;
			v.p = vec3f(pts[i].x, pts[i].y, pts[i].z);
